...
{
	// save area and recalculate bounds with margin when all the book pages
	// are loaded to take the book aspect ratio into account
	book_area = area;
	recalc_bounds_needed = true;
}

void PageCurl::recalc_bounds()
{
	float b_aspect = book_area.getWidth() / static_cast<float>(book_area.getHeight());
	if (b_aspect > book_aspect_ratio)
	{
		int margin = (book_area.getWidth() - book_area.getHeight() * book_aspect_ratio) / 2;
		book_area.expand(-margin, 0);
	}
	else
	{
		int margin = (book_area.getHeight() - book_area.getWidth() / book_aspect_ratio) / 2;
		book_area.expand(0, -margin);
	}

	bounds = book_area;
	pdim = Vec2f(bounds.getWidth() / 2.0, bounds.getHeight());

	recalc_bounds_needed = false;
}
...
void PageCurl::load_page(const string filename)
{
	...
	{
		t = gl::Texture(loadImage(filename));
	}
	...
	// cache texture in gpu
	gl::draw(t);
}

bool PageCurl::preload(float *progress, int files_per_call /* = 5 */)
{
	bool finished = false;
	...
	{
		console() << book_files.size() << endl;
		if (!book_files.empty())
		{
			for (int i = 0; i < files_per_call; i++)
			{
				if (book_files.empty())
				{
					finished = true;
	...
	if (finished)
	{
		...
		book_aspect_ratio = 2 * book[0].getWidth() / static_cast<float>(book[0].getHeight());
	}

	gl::clear();

	*progress = book.size() / (float)(book_files.size() + book.size());
	return finished;
}
...
void PageCurl::render(const Vec2f& pos, const int page0_i, const int corner)
{
	...
	{
		Vec2f zpos(pdim.x, pdim.y / 2);
		zpos += zpos * pzoom_pos;

		gl::translate(zpos);
		gl::scale(Vec3f(pzoom, pzoom, pzoom));
		gl::translate(-zpos);
	}
