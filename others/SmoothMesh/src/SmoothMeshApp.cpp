/*
 * SmoothMesh by Paul Houx
 * https://forum.libcinder.org/#Topic/23286000001327009
 */
...
void SmoothMeshApp::generateNormals()
{
	vector< Vec3f > normals;
	normals.resize( mTriMesh.getNumVertices() );

	const vector< Vec3f > & vertices =	mTriMesh.getVertices();

	const std::vector< uint32_t > & indices = mTriMesh.getIndices();
	...
	{
		Vec3f v0 = vertices[ indices[i * 3] ];
		Vec3f v1 = vertices[ indices[i * 3 + 1] ];
		Vec3f v2 = vertices[ indices[i * 3 + 2] ];
	...
	{
		mTriMesh.appendNormal( normals[i].normalized() );
	}
}
